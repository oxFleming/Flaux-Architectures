  ...
  return (
    <section className="bg-white border-t border-black/5 scroll-mt-28" id="testimonials">
      <div className="flex flex-col lg:flex-row min-h-[700px]">
        ...
        <div className="w-full lg:w-[40%] bg-flaux-dark text-white p-12 lg:p-24 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
             <div className="flex items-center gap-3 mb-8">
                <span className="h-px w-8 bg-flaux-red"></span>
                <span className="text-flaux-red font-bold uppercase tracking-widest text-xs">Client Voices</span>
             </div>
          ...
          <div className="relative z-10 mt-12 lg:mt-0">
             <p className="text-white/70 text-lg leading-relaxed max-w-sm border-l-2 border-flaux-red pl-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 h-full">
            {testimonials.map((item, index) => (
              <div 
                key={item.id} 
                className={`
                    p-12 lg:p-16 flex flex-col justify-between 
              ...
              >
                <div>
                  ...
                  <p className="text-xl font-serif text-gray-800 leading-relaxed mb-8 italic">
                    "{item.text}"
                  </p>
                </div>
                
                <div className="pt-8 border-t border-black/5 group-hover:border-flaux-red/30 transition-colors">
                  <h4 className="font-bold text-black text-xs uppercase tracking-widest">{item.author}</h4>
                  {item.role && (
                    <span className="text-gray-400 text-xs mt-1 block font-serif italic">{item.role}</span>
                  )}
                </div>
              </div>
            ))}
